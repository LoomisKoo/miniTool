...
enum ImageImport {
    /// 相册数据 → 方向已归一的 CGImage。
    ///
    /// `UIImage.cgImage` 会丢掉 EXIF 方向，直接裁切会把竖拍照片画横，所以先重绘一遍。
    ///
    /// - Parameter maxSide: 最长边上限。裁图要保留细节，默认 4096（约 64MB 位图内存）。
    static func cgImage(from data: Data, maxSide: CGFloat = 4096) throws -> CGImage {
        guard let image = UIImage(data: data) else { throw ImageImportError.decodeFailed }
        return try normalized(image, maxSide: maxSide)
    }

    static func normalized(_ image: UIImage, maxSide: CGFloat = 4096) throws -> CGImage {
