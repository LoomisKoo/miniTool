#!/usr/bin/env node
/**
 * 打包微风铃语 — 小红书 / 抖音 / 快手（离线 zip，禁 CDN）
 * 用法:
 *   node build.mjs
 *   node build.mjs xiaohongshu douyin kuaishou
 */
import fs from 'fs';
...
const ALL = ['xiaohongshu', 'douyin', 'kuaishou'];
...
// 音频 + 光效 + 主程序合并为单文件 app.js
const audio = fs.readFileSync(path.join(__dirname, 'src/audio.js'), 'utf8');
const fx = fs.readFileSync(path.join(__dirname, 'src/fx.js'), 'utf8');
const main = fs.readFileSync(path.join(__dirname, 'src/app.js'), 'utf8');
const app = [audio, fx, main].join('\n\n');
const three = fs.readFileSync(path.join(__dirname, 'vendor/three.min.js'), 'utf8');
...
<meta name="theme-color" content="#0d0f2a">
...
`;
  ...
  execSync(`zip -q -r "${zipPath}" index.html style.css app.js three.min.js assets -x '*.DS_Store'`, {
    cwd: outDir,
    stdio: 'pipe'
  });
