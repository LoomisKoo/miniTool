...
struct Bead3DMetalCanvas: UIViewRepresentable {
    ...
    /// 当前外观：豆的淡化色在 CPU 侧算进实例缓冲，所以它变了要重建缓冲；
    /// 底板色不进缓冲、每帧现算（见 `plateTilt`）。
    var appearance: BeadAppearance = .light
    /// 2D↔3D 过渡里「立起来」的进度（0 = 正俯视）。底板色每帧按它算：
    /// 正俯视时贴预览框底色，淡入那一下才不会是「整幅翻黑」。
    ...
    func updateUIView(_ view: MTKView, context: Context) {
        ...
        coordinator.plateTilt = plateTilt
    ...
    final class Coordinator: NSObject, MTKViewDelegate {
        ...
        /// 当前外观：底板色每帧要按它 + `plateTilt` 现算（不进缓冲，见 `draw(in:)`）。
        private var appearance: BeadAppearance = .light

        var camera = Bead3DCamera()
        /// 「立起来」的进度，决定底板色（0 = 贴预览框底色）。
        var plateTilt: Double = 1
        ...
        func update(
            ...
            signature = next
            rebuild(grid: grid, rect: rect, highlightedCode: highlightedCode, appearance: appearance)
        }
        ...
        private func rebuild(
            ...
            for row in 0..<rect.height {
                for column in 0..<rect.width {
                    ...
                    let rgb = dimmed
                        ? cell.rgb.mixed(with: appearance.dimTarget, t: 0.72)
                        : cell.rgb
            ...
            // 底板色不进缓冲：它每帧都可能被过渡进度改（见 `draw(in:)`），
            // 用一个 16 字节的 vertex bytes 直接喂，省掉「共享缓冲边写边读」的隐患。
        ...
        func draw(in view: MTKView) {
            ...
            if let plateVertexBuffer, let originBuffer {
                // 底板色每帧现算：过渡期「正俯视」时它贴预览框底色，
                // 立起来的过程中再压到近黑，淡入那一下就不会整幅翻黑。
                let plate = Bead3DRenderer.plateRGB(appearance, tilt: plateTilt)
                var plateRGBA: [Float] = [
                    Float(plate.r) / 255,
                    Float(plate.g) / 255,
                    Float(plate.b) / 255,
                ...
                encoder.setVertexBytes(&plateRGBA, length: plateRGBA.count * MemoryLayout<Float>.stride, index: 2)
