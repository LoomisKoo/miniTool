...
struct BeadProject: Codable, Identifiable, Hashable {
    ...
    /// 列表副标题：尺寸 + 板数。
    var summary: String {
        "\(gridWidth)×\(gridHeight) · \(settings.beadWidth) 豆宽 · \(settings.maxColors) 限色"
    }
...
/// 作品落盘位置：`Application Support/BeadPattern/`。
///
/// 不放 Caches：系统在空间紧张时会清空 Caches，用户的作品不能这么丢。
enum ProjectPaths {
    static let root: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        return base.appendingPathComponent("BeadPattern", isDirectory: true)
    }()
