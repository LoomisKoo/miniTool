...
| 能力 | 说明 |
| --- | --- |
| **Storage JS API（首选）** | `window.xhs.miniTool.setStorage` / `getStorage` / `getStorageInfo` / `removeStorage` / `clearStorage`；需客户端 ≥ **9.46.0**，见 [jsbridge-api.md](./jsbridge-api.md#storage--小工具本地缓存) |
| `localStorage` / `sessionStorage` / `IndexedDB` / `Cookie` / `Cache API`（降级） | 仅在客户端版本不满足 Storage 要求时使用，容器**不保证可用或持久** |

- 持久化**优先 Storage JS API**，浏览器自带存储只作低版本降级；不要把它当可靠方案。
- 降级路径的读写都要 `try/catch`，并能容忍数据缺失 / 失效 / 被清理；迁移与一致性自己维护。
- 单 key ≤ 1MB，当前小工具全部缓存 ≤ 10MB。
- 数据仅属于当前小工具，其他小工具与外部无法访问，不要假设永久持久化。

Cookie 仅作本地存储：可读写、按 origin 隔离，但因不联网**不会随请求发往服务端**，不能用于登录态 / 鉴权。
...
| 发笔记 | `postNote.mediaInfo` 必填；图片走 `image_resources[].url`，视频走 `video_resources`，实况走 `live_photo_sources`（客户端 9.43+） |
| 跳原生页 | `openRedPage.type` 须命中 Native 规则表白名单，`params` 为语义参数 |
| 版本判断 | 同步优先 `window.xhs.launchOptions.miniToolEnv.buildVersion`，取不到再 `getLaunchOptions()`；`buildVersion` 末 3 位是编译号，比较版本时忽略 |
| 本地缓存 | Storage JS API（`setStorage` / `getStorage` / `getStorageInfo` / `removeStorage` / `clearStorage`），需客户端 ≥ 9.46.0；低版本降级见 §1「数据存储」 |
...
| 存储进阶 | `navigator.storage.persist`（持久化）、跨域存储访问 | 移除；持久化改用 Storage JS API（见 §1 数据存储），客户端 < 9.46.0 才降级 `localStorage` |
...
| 文件下载 | `a[download]`、blob 下载 | 保存图片改用 `writeTempFile` → `saveImageToPhotosAlbum`，见 [jsbridge-api.md](./jsbridge-api.md) |
...
| 本地持久化（配置 / 收藏 / 最高分） | `setStorage` / `getStorage`（客户端 ≥ 9.46.0），低版本降级 `localStorage`；key 带业务前缀，读写都要异常兜底 |
| 按客户端版本决定功能开关 | 同步读 `window.xhs.launchOptions.miniToolEnv.buildVersion`，取不到再 `getLaunchOptions()`；忽略末 3 位编译号 |
...
window.xhs.miniTool.*                                     // 端能力：发笔记 / 存相册 / 临时文件 / 版本 / 本地缓存
localStorage / sessionStorage / IndexedDB / Cookie / Cache API   // 仅作 Storage JS API 的降级，不保证持久
alert() / confirm()
