...
3. **Native 能力（JSBridge）** — 需要发笔记、存相册、跳原生页、本地持久化（Storage）或判断客户端版本时，先读 [jsbridge-api.md](references/jsbridge-api.md)：仅使用文档列出的 `window.xhs.miniTool.*` API，参数与必填项以该文档为准，未列出的字段不要传；持久化优先用 Storage JS API，`localStorage` 只作低版本降级
...
| [jsbridge-api.md](references/jsbridge-api.md) | 调用 Native 能力时：`window.xhs.miniTool.*` 全量 API（发笔记 / 存相册 / 临时文件 / 版本判断 / Storage 本地缓存）、参数约束、示例与常见组合 |
