    (function () {
      ...
      function renderCropChips(animateSeg) {
        ...
        SHAPES.forEach(function (s) {
          ...
          btn.addEventListener('click', function () {
            ...
            radiusBar.style.display = s.id === 'round' ? 'flex' : 'none';
            renderCropChips(true);
            animateCropFrame();
          });
        ...
        radiusBar.style.display = state.shapeId === 'round' ? 'flex' : 'none';
      ...
      function isImageFile(file) {
        if (!file) return false;
        if (isProbablyVideo(file)) return false;
        var type = (file.type || '').toLowerCase();
        if (type.indexOf('image/') === 0) return true;
        var name = (file.name || '').toLowerCase();
        if (/\.(jpe?g|png|gif|webp|bmp|heic|heif)$/.test(name)) return true;
        // 无 MIME、无扩展名：仍尝试当图片（不少相册如此），靠超时兜底
        if (!type && (!name || name.indexOf('.') === -1)) return true;
        return false;
      }
      ...
      function drawCrop() {
        ...
        if (grid.cols * grid.rows > 1 && state.shapeId === 'rect') {
          cropCtx.strokeStyle = state.gridLineColor || 'rgba(255,255,255,0.75)';
      ...
      function onFilesSelected(files, append) {
        ...
        if (!imagesOnly.length) {
          if (!videoCount) {
            showToast(otherSkip ? '请选择图片文件' : '请选择图片');
          }
          fileInputMulti.value = '';
          return;
        }

        var room = append ? (9 - state.images.length) : 9;
        if (room <= 0) {
          showToast('最多选择 9 张图片');
          fileInputMulti.value = '';
          return;
        }
        list = imagesOnly.slice(0, room);
        ...
        Promise.all(list.map(function (file) {
          ...
          if (append) {
            state.images = state.images.concat(imgs).slice(0, 9);
            syncCropImage();
            if (state.mode === 'stitch') {
              renderStitchPreview();
              subtitle.textContent = '已选 ' + state.images.length + ' 张，长按可调顺序';
          ...
          // 首页入口 / 重选：按指定模式进入（用进入时锁定的 mode，避免异步丢失）
